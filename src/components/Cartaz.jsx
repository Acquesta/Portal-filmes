...
function Cartaz() {
    ...
    const [bgCartaz, setBgCartaz] = useState('')
    ...
    useEffect(() => {
        ...
        .then(data => {
            ...
            setBgCartaz(data.results[0].backdrop_path)
        })
    ...
    const trocaCartaz = (backdrop) => {
        console.log(backdrop);
        setBgCartaz(backdrop)
    }
                ...
                <div className="flex gap-5 ml-5 mt-[-10rem] overflow-scroll overflow-y-hidden scroll">
                    {cartaz.map(filme => (
                        <div key={filme.id} className="w-[90vw] my-10 mx-auto gap-2 cursor-pointer">
                            <img onClick={() => trocaCartaz(filme.backdrop_path)} className="min-w-[10vw] min-h-[10vw] rounded-xl" src={`https://image.tmdb.org/t/p/w500${filme.poster_path}`}s alt="" />
                        </div>
                    ))}
                </div>
            </div>
